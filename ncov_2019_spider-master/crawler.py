...
class Crawler:
    ...
    def crawler(self):
        ...
        while True:
            ...
            self.crawl_timestamp = time.strftime("%Y-%m-%d %H:%M:%S",time.localtime(int(str(int(datetime.datetime.timestamp(datetime.datetime.now()) * 1000))[0:-3])))
            try:
                r = self.session.get(url='https://3g.dxy.cn/newh5/view/pneumonia')
            except requests.exceptions.ChunkedEncodingError:
                continue
            soup = BeautifulSoup(r.content, 'lxml')
            overall_information = re.search(r'\{("id".*?)\}\}catch',
                                            str(soup.find('script', attrs={'id': 'getStatisticsService'})))
            # print(str(soup.find('script', attrs={'id': 'getStatisticsService'})))
    ...
    def overall_parser(self, overall_information):
        ...
        overall_information['countRemark'] = overall_information['countRemark'].replace(' 疑似', '，疑似').replace(' 治愈','，治愈').replace(
            ' 死亡', '，死亡').replace(' ', '')
        if not self.db.find_one(collection='DXYOverall', data=overall_information):
            ...
            # self.db.update(collection='DXYOverall',myquery=overall_information,newvalues=overall_information)
